...
class _StatisticsScreenState extends State<StatisticsScreen>
  ...
  Widget _buildTabBar() {
    return Container(
      ...
      child: TabBar(
        ...
        tabs: const [Tab(text: 'Personal'), Tab(text: 'LeetCode')],
      ),
    );
  }
  ...
  Widget _buildWeeklyRequirements(LeetBlockProvider provider) {
    ...
    for (var date in dates) {
      ...
      final isComplete =
          (key == todayKey) ? provider.isBaseQuotaMet : (history[key] == true);
  ...
  Widget _buildScreenTimeGraph(LeetBlockProvider provider) {
    ...
    final endDate = now.subtract(
      Duration(
        days:
            _screenTimePeriod == '1W'
                ? _screenTimePageOffset * 7
                : _screenTimePageOffset * 30,
      ),
    );
    ...
    final dates = List.generate(
      daysPerPeriod,
      (index) => endDate.subtract(Duration(days: daysPerPeriod - 1 - index)),
    );
      ...
      for (int i = 0; i < dates.length; i++) {
        if (_dateKey(dates[i]) == todayKey) {
          if (history[todayKey] == null ||
              provider.totalBlockedTime > history[todayKey]!) {
                    ...
                    children: List.generate(data.length, (index) {
                      ...
                      return Expanded(
                        child: GestureDetector(
                          ...
                          onTapDown:
                              (_) => setState(() => _hoveredBarIndex = index),
                          onTapUp:
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                margin: EdgeInsets.symmetric(
                                  horizontal: isWeek ? 8 : 1,
                                ),
                                decoration: BoxDecoration(
                                  color:
                                      (isHovered || isSelected)
                                          ? const Color(0xFFFFA116)
                                          : const Color(0xFFFFA116).withOpacity(
                                            0.5 + (heightFactor * 0.5),
                                          ),
                                  borderRadius: BorderRadius.circular(4),
                                  border:
                                      isSelected
  ...
  Widget _buildAppUsageList(LeetBlockProvider provider) {
    ...
    final List<MapEntry<String, int>> sortedApps =
        appsForDate.entries.toList()
  ...
  Widget _buildLeetCodeActivityChart(LeetBlockProvider provider) {
    ...
    final (chartData, totalInPeriod, periodLabel) = _getLeetCodeChartData(
      detailedStats,
      _leetcodeChartPeriod,
    );
          ...
          SizedBox(
            ...
            child: LayoutBuilder(
              builder: (context, constraints) {
                final singlePoint = chartData.length <= 1;
                // Determine point width (avoid divide-by-zero for single-point series)
                final pointWidth =
                    singlePoint
                        ? constraints.maxWidth
                        : constraints.maxWidth / (chartData.length - 1);

                return GestureDetector(
                  onPanUpdate: (details) {
                    if (singlePoint) {
                      setState(() => _leetcodeHoverIndex = 0);
                      return;
                    }
                    final index = (details.localPosition.dx / pointWidth)
                        .round()
                        .clamp(0, chartData.length - 1);
                  ...
                  child: Stack(
                    ...
                    children: [
                      ...
                      if (_leetcodeHoverIndex != null &&
                          _leetcodeHoverIndex! < chartData.length)
                        Builder(
                          builder: (context) {
                            final value = chartData[_leetcodeHoverIndex!];
                            final maxVal = chartData.reduce(math.max);
                            final normalizedY =
                                maxVal > 0 ? (1 - value / maxVal) * 100 : 100.0;
                            final pointX =
                                singlePoint
                                    ? constraints.maxWidth / 2
                                    : _leetcodeHoverIndex! * pointWidth;

                            // Calculate date for this index
                            final daysAgo =
                                chartData.length - 1 - _leetcodeHoverIndex!;
  ...
  (List<double>, int, String) _getLeetCodeChartData(
    ...
    if (calendarJson is String) {
      try {
        final Map<String, dynamic> decoded = Map<String, dynamic>.from(
          java_convert.jsonDecode(calendarJson),
        );
      ...
      for (int i = 6; i >= 0; i--) {
        ...
        calendar.forEach((ts, count) {
          ...
          if (date.year == day.year &&
              ...
              date.day == day.day) {
            data[6 - i] +=
                count; // Using i here means rightmost is i=0 (today). index logic: 6-i
            total += count;
          }
        });
      }
  ...
  Widget _buildDifficultyRow(String label, int solved, int total, Color color) {
    final percentage = total <= 0 ? 0.0 : (solved / total).clamp(0.0, 1.0);
  ...
  Widget _buildRecentActivity(
    ...
    final rawRecentProblems = provider.detailedStats?['recentProblems'];
    final recentProblems =
        rawRecentProblems is List
            ? rawRecentProblems
                .whereType<Map>()
                .map(
                  (item) =>
          ...
          (context) => AlertDialog(
            ...
            actions: [
              TextButton(
                ...
                child: Text(
                  'Close',
                  style: GoogleFonts.inter(color: const Color(0xFFFFA116)),
                ),
              ),
            ],
          ),
...
class AreaChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (data.length == 1) {
      final x = width / 2;
      final y = height - (data.first / maxVal * height);
