...
class _SettingsScreenState extends State<SettingsScreen> {
                ...
                _buildSettingTile(
                  ...
                  onTap:
                      () => Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (_) => const PenaltySettingsScreen(),
                        ),
                      ),
                ),
          ...
          if (provider.currentStats != null) ...[
            ...
            Row(
              ...
              children: [
                ...
                _buildMiniStat('Easy', '${provider.currentStats!.easySolved}'),
                _buildMiniStat(
                  'Medium',
                  '${provider.currentStats!.mediumSolved}',
                ),
                _buildMiniStat('Hard', '${provider.currentStats!.hardSolved}'),
              ],
            ),
          ],
        ],
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Center(
            child: Text(
              currentQuota == 1 ? 'problem per day' : 'problems per day',
          ...
          Center(
            child: TextButton(
              key: const ValueKey('settings_edit_todays_progress_button'),
  ...
  void _showTodaysProgressSheet(
    ...
    showModalBottomSheet(
      ...
      builder:
          (context) => Consumer<LeetBlockProvider>(
            builder: (context, provider, _) {
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        IconButton(
                          key: const ValueKey(
                            'settings_progress_decrement_button',
                          ),
                          onPressed:
                              completed > 0
                          ...
                          icon: Icon(
                            Icons.remove,
                            color:
                                completed > 0 ? Colors.white70 : Colors.white24,
                            size: 28,
                          ),
                        ),
                        ...
                        IconButton(
                          key: const ValueKey(
                            'settings_progress_increment_button',
                          ),
                    ...
                    AnimatedOpacity(
                      ...
                      child: Column(
                        children: [
                          ...
                          TextButton(
                            key: const ValueKey(
                              'settings_progress_reset_offset_button',
                            ),
                            onPressed:
                                offset != 0
                                    ? () => provider.resetManualOffset()
                                    : null,
                            child: Text(
                              'Reset adjustment',
                              style: GoogleFonts.inter(
  ...
  Future<void> _showBlockMessageDialog(
    BuildContext context,
    LeetBlockProvider provider,
  ) async {
    final message = await showDialog<String>(
      ...
      builder:
          (context) =>
              _BlockMessageDialog(initialMessage: provider.blockMessage),
    );
    if (message == null) {
      return;
    }
    provider.setBlockMessage(message.trim().isEmpty ? 'LOCK IN' : message);
  }
  ...
  Future<void> _showResetConfirmation(
    ...
    if (settingsNavigator.canPop()) {
      settingsNavigator.pop();
    }
  }
}

class _BlockMessageDialog extends StatefulWidget {
  final String initialMessage;

  const _BlockMessageDialog({required this.initialMessage});

  @override
  State<_BlockMessageDialog> createState() => _BlockMessageDialogState();
}

class _BlockMessageDialogState extends State<_BlockMessageDialog> {
    ...
    return AlertDialog(
      ...
      content: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          TextField(
            key: const ValueKey('settings_reset_confirmation_input'),
            controller: _controller,
            style: GoogleFonts.inter(color: Colors.white),
            decoration: InputDecoration(
              filled: true,
      ...
      actions: [
        ...
        TextButton(
          key: const ValueKey('settings_reset_everything_button'),
          onPressed: () {
            final confirmation = _controller.text.trim().toLowerCase();
